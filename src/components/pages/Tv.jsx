...
export default function TV() {
  ...
  const{user,session,favoriteTvs,fetchFavoriteTvs} = useContext(UserContext);
  console.log(favoriteTvs)

  useEffect(()=>{
    if(tv && favoriteTvs.length){
      const favtvs = favoriteTvs.find((f)=>f.id===tv?.id);
      setIsfavorite(Boolean(favtvs));
      console.log(favtvs)
  ...
  async function handlefavorite(){
    if(session){
      ...
      toast.success(`${tv.name} ${isfavorite?'Removed':'Added'} to your favorite watchlist`)
  }else{
    toast.error("Error")
  }
}
  ...
  async function loadTv(){
      const {data} = await axios.get(`https://api.themoviedb.org/3/tv/${id}?api_key=f44402f79564f6b266de86037898fe8e`);
      setTv(data);
  }
  useEffect(()=>{
       loadTv()
        ...
        tv ? (
          <div className='container grid grid-cols-4 gap-10'>
            <div className="col-span-1">
              <img src={`http://image.tmdb.org/t/p/w780/${tv.poster_path}`} alt={tv.title}  />
            ...
            <div className="col-span-3">
              ...
              <div className='grid grid-cols-4 border-slate-500 border-t-2 border-b-2 mt-4'>
                 <div className='col-span-1 border-r-2 border-slate-500 flex items-center text-slate-300 py-1'>
                   <div className='flex items-center gap-3'>
                     ...
                     <div className='flex flex-col gap-2'>
                       <div>{parseInt(tv.vote_average)} / 10</div>
                 ...
                 <div className='col-span-3 pl-2 flex gap-4 items-center text-slate-300'>
                   Rate this movie
                    <ReactStars
                     value={parseInt(tv.vote_average ) / 2}
                     count={5}
